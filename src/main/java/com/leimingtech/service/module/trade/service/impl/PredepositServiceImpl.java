...
public class PredepositServiceImpl implements PredepositService{
	 ...
	 */
	public PredepositRecharge addRechargePredeposit(String memberId,BigDecimal pdrAmount){
		try{
			...
			predepositLog.setLgAddAmount(BigDecimal.valueOf(0));//存入余额为0表示未变更
			predepositLog.setLgRdeAmount(BigDecimal.valueOf(0));//支出余额为0表示未变更
		...
		try{
			...
			if(predepositRecharge!=null){ 
				...
				if(predepositRecharge.getPdrPaymentState().equals("0")){ //未支付
					...
					predepositLog.setLgAddAmount(predepositRecharge.getPdrAmount()); //充值金额为存入余额
					predepositLog.setLgFreezeAmount(BigDecimal.valueOf(0)); //冻结金额变更0表示未变更
	 ...
	 * @param paymentId
	 */
	public void updateRechargePredeposit(String pdrSn,String paymentId){
		try{
